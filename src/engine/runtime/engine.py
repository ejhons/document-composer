...
from engine.common.exceptions import ResolutionException
from engine.common.models.recipe import RecipeManifest
from engine.frontend.inspection_pipeline import InspectionPipeline
from engine.frontend.syntax.markdown.fragmented import MarkdownAtomizer
from engine.frontend.syntax.markdown.fragments import MarkdownDirective, MarkdownPlaceholder
...
class Engine:
    ...
    def _bootstrap_adapters(self):
        ...
        self.adapter_registry.register_adapter("xlsx", ExcelToMarkdownAdapter())
        self.adapter_registry.register_adapter("pdf", PdfToImageMarkdownAdapter())
        self.adapter_registry.register_adapter("docx", DocxPostCompileAdapter())
        self.adapter_registry.register_adapter("mermaid", MermaidMarkdownAdapter(
            self.cache
        ))
        self.adapter_registry.register_adapter("md", MarkdownAdapter(
    ...
    def build_graph(
        ...
        graph = RecipeGraphBuilder(
            context=self.plan_context#session.execution_context
        ).build(
...
#             if component.solvable:  #.type == "template":
