...
from dcp_engine.solving.resolution.adaptation_resolver import AdaptationResolver
...
""",
    ...
    graph = RecipeGraphBuilder(resource_resolver).build(recipe_manifest)

    adapter = FakeMarkdownAdapter()

    planning_context.adapter_registry.register("md", adapter)
    ...
    solver = AdaptationResolver(adapter_registry=planning_context.adapter_registry)  
